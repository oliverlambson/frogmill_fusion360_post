...
description = "RS-274D - FrogMill";
vendor = "Autodesk (modified by Oliver Lambson)";
vendorUrl = "http://www.autodesk.com";
legal = "Copyright (C) 2012-2016 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 24000;

longDescription = "Post for Streamline Automation FrogMill using a modified generic RS-274D post (provided by Autodesk).";
...
properties = {
  ...
  preloadTool: false, // preloads next tool on tool change if any [OCL]
  showSequenceNumbers: true, // show sequence numbers
  sequenceNumberStart: 0, // first sequence number [OCL]
  sequenceNumberIncrement: 1, // increment for sequence numbers [OCL]
...
// 0,    1,    2,    3,    4,    5, 6
  [9, null, null, null, null, null, 8],
  {initial:COOLANT_OFF, force:true},
  "Invalid coolant mode"
// User Interface Label     Post Description            onParameter String value    Array Position      M-code (mapped table)
// Disabled                 COOLANT_OFF                 disabled                    0                   M09
// Flood                    COOLANT_FLOOD               flood                       1                   null
// Mist                     COOLANT_MIST                mist                        2                   null
// Through tool             COOLANT_THROUGH_TOOL        tool                        3                   null
// Air                      COOLANT_AIR                 air                         4                   null
// Air through tool         COOLANT_AIR_THROUGH_TOOL    air through tool            5                   null
// Suction                  COOLANT_SUCTION             suction                     6                   M08
// Flood and mist           COOLANT_FLOOD_MIST          flood mist                  7                   null
// Flood and through tool   COOLANT_FLOOD_THROUGH_TOOL  flood tool                  8                   null
);
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    writeBlock(gFormat.format(28)); // go to tool change position
    zOutput.reset();
  }
  ...
  if (insertToolCall ||
    ...
    if (tool.spindleRPM > 18000) { // [OCL]
      warning(localize("Spindle speed exceeds maximum value."));
    }
    writeBlock(
      sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4)
    );
    writeBlock(gFormat.format(4), "P" + secFormat.format(4000)); //pause for spindle to get up to speed
  }
...
function onClose() {
  ...
  onCommand(COMMAND_STOP_SPINDLE); // OCL added 11/08/2018
